...
- Ports free: `8090` (app), optionally `9000`/`9001` (MinIO)
...
Put nginx/Caddy in front of `:8090` and terminate TLS. Set `CORS_ORIGIN` and `R2_PUBLIC_ENDPOINT` to the public origins users will use.

Upgrade to a known git SHA (this is how VPS, Pages, and git stay on one build):

```bash
git fetch origin
git checkout <sha-or-branch-with-the-chat>
# keep deploy/vps/.env
bash scripts/vps-up.sh
```

The studio header shows the baked git SHA. If it does not match `git rev-parse HEAD`, the image is stale.
